...
mnist_mlp = tf.keras.models.Sequential([
    ...
    tf.keras.layers.Dense(10, activation='softmax')]) # Strato di output per classificcazione multicl
...
n_epochs = 25
mnist_mlp_history = mnist_mlp.fit(x_train_mnist_norm, y_train_mnist_enc, validation_split=0.2, epochs=n_epochs, verbose=1)

# Valutazione del modello
mnist_test_loss, mnist_test_accuracy = mnist_mlp.evaluate(x_test_mnist_norm, y_test_mnist_enc, verbose=0)
print(f"Loss sul set di test: {mnist_test_loss}")
print(f"Accuracy sul test set: {mnist_test_accuracy}")
...
# Salvare le metriche, le etichete vere e predette
results ={
    'history': mnist_mlp_history.history,
    'test_loss': mnist_test_loss,
    'test_accuracy': mnist_test_accuracy,
    'true_labers': y_test_mnist.flatten(),
    'predicted_labers': y_pred_mnist,
